		...
		<view class="u-tooltip__wrapper">
			...
			>
				...
				>
					...
					<view class="u-tooltip__wrapper__popup__list flex-column" style="min-height: 30rpx;min-width: 100rpx">
						<view v-for="(item , index) in buttons" :key="index" class="flex-column" @click="()=>{
							$emit('click',{item,index});
							overlayClickHandler();
						}">
							<slot name="buttonItem"  v-bind:item="item"/>
							<u-line direction="row" color="#8d8e90" v-if="index < buttons.length - 1" length="100%"/>
						</view>
					</view>
				</view>
			</u-transition>
...
<script>
	...
	export default {
		...
		computed: {
			...
			tooltipStyle() {
				const style = {
						transform: `translateY(${this.direction === 'top' ? `-${this.buttonHeight}` : this.buttonHeight})`,
					},
					sys = uni.$u.sys(),
					getPx = uni.$u.getPx,
				...
				if (this.tooltipInfo.width / 2 > this.textInfo.left + this.textInfo.width / 2 - this.screenGap) {
					...
					this.indicatorStyle.right = addUnit(this.textInfo.width / 2 - getPx(style.right) - this
						.indicatorWidth / 2 +this.screenGap)
				} else {
				...
				if (this.direction === 'top') {
					...
					style.marginTop = '5px';
					this.indicatorStyle.top = '-3px';
				}
				if (this.right){
				    style.right ='1px';
                    this.indicatorStyle.right ='6px';
                }
				return style
			}
		},
